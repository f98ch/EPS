...
{Implementación de canales cuánticos \\ de un\textit{qubit} usando VQA}%		% Título del trabajo de graduación
...
\begin{itemize}
    ...
    \item Implementar canales cuánticos de un\textit{qubit} en computadoras cuánticas y analizar su comportamiento.
\end{itemize}
% }}}
...
El \textit{\textit{depolarizing channel}} es un ejemplo de canal cuántico que introduce ruido aleatorio en un sistema. Su matriz de Choi refleja cómo el canal mezcla el estado original con la matriz identidad, reduciendo la pureza del estado. \par 
...
Previo a hablar sobre compuertas y circuitos cuánticos, introducimos la Esfera de Bloch para representar geométricamente al\textit{qubit}. Esta esfera nos permite visualizar los cambios del estado de un\textit{qubit} bajo transformaciones unitarias $U$. \par 

Las computadoras cuánticas tienen compuertas cuánticas, operadores unitarios reversibles $U$ que actúan linealmente sobre\textit{qubit}s en superposición. Las compuertas $X$, $Y$ y $Z$ representan las matrices de Pauli $\sigma_x$, $\sigma_y$ y $\sigma_z$ respectivamente. \par 

Un circuito cuántico es una rutina computacional con operadores que actúan sobre\textit{qubit}s. Los circuitos se conforman por cables, compuertas y mediciones. \par 
...
\section{Implementación de canales de un\textit{qubit} usando VQA}
...
Hemos propuesto utilizar los VQA para inicializar canales cuánticos de 1\textit{qubit}, para esto , nuestra función de costo a optimizar será la fidelidad de proceso entre la matriz de Choi teórica contra la reconstruida en una computadora cuántica. \par 
 

% }}}
\chapter{RESULTADOS PARA EL DEPOLARIZING CHANNEL DE UN QUBIT } % {{{
\section{Introducción} % {{{
\cpnote{Falta introduccion. Escribirla al final quizá de la tesis}\felnote{Ok. La redactaré al final}
% }}}
\section{Algoritmos de implementación} % {{{
Se han programado las funciones necesarias para implementar un
\textit{depolarizing channel} \cpnote{para evitar el uso del ingle, de propongo
que pongas algo como depolarizing channel (DP) y que luego te refieras a el
como DP.} mediante VQA en una computadora cuántica. Estas funciones siguen el
...
corresponde al \textit{depolarizing channel} con probabilidad de depolarización $p$.
...
acorde a la teoría explicada en el \autoref{cap:fundamentos:teoricos}\cpnote{Le cambié el nombre
a la referencia. Si decides por ejemplo que la intro lleva numero, luego ese sería el 
dos y seria confuso que el dos se referencie como uno. Checalo en el codigo y borra 
esta nota}. Toma como entrada el circuito
...
capítulo 1\cpnote{De nuevo referencia abstracta aca. Aparte, convendría 
aca referenciar la ecuacion exacta que necesites}.
...
defecto es para dos\textit{qubit}s) \cpnote{no estas usando italicas antes para
qubits. Creo qeu es mejor no usarlas}.
...
ejecutar la tomografía de proceso sobre el primer\textit{qubit} del circuito generado.
...
costo converja \cpnote{mas bien que minimicen la funcion de costo, no? Casi todos
los parametros iniciales van a converger}. 
...
\vspace{-.5cm}
\noindent\rule{\textwidth}{1mm}
\cpnote{No veo el uso de las lineas gruesas. Las quitaria}
% }}}
\section{Resultados} % {{{
...
parámetro\cpnote{Tocaría mostrar una gráfica donde se demuestre esa
afirmación.}. Se utilizaron 4 \textit{qubits} en el \textit{ansatz}, donde dos se usaron como
\textit{ancillas} y otros dos para el sistema, siguiendo el diseño presentado en el
capítulo anterior. Esto se decidió porque al utilizar únicamente dos \textit{qubit}s no
se alcanzó la convergencia en la función de costo, y al utilizar 6 o más \textit{qubit}s
tampoco se notó una mejora notoria. \cpnote{no porqué pero cuando tienes qubit a
veces no separas el espacio entre la palabra anteiror y qubit. Creo que es
porque cambiaste automaticamente. }
...
tiempos de ejecución ni en la optimización de la función de costo\cpnote{frase
...
tiempo de ejecución de una tomografía de proceso para un canal de un \textit{qubit}
\cpnote{revisa todas las instancias de qubit y dejalo sin italicas por favor}
...
número de 4000 \textit{shots}\cpnote{demasiados anglisismos. Solo usalos si la 
traduccion es rara. aca puedes aclarar la situacion en español} o mediciones,
...
correspondiente al \textit{depolarizing channel}. Sabemos que su matriz de Choi esta
...
identidad $I$. \cpnote{Aca si puede haber un problema más serio. Lo platicamos.}
\begin{figure}[h!] 
    \centering 
...
\cpnote{A ver si lo justificas mas adelante. Se me hace una frase osada}
...
de $\chi^2 = 0.1085$. Esta ecuación indica que la fidelidad en la computadora
cuántica aumenta proporcionalmente a $p$\cpnote{no, tu propusiste un crecimiento lineal. 
y la verdad no da tan bien. mejorra la interpretacion. quizá es mejor si discutimos acerca 
de estas graficas}. Este comportamiento podría deberse a
que, al aumentar la probabilidad de depolarización, también se incrementa la
decoherencia en el sistema\cpnote{Interpretacion chafa}. En consecuencia, la computadora cuántica
\texttt{ibm\_kyoto} parece ofrecer mejores resultados de fidelidad cuando hay
...
Sería bueno citarlos. Hablemos también}
...
\cpnote{Me falta corregir este parrafo pues no entiendo la figura}
...
\begin{figure}[h!]
    ...
    \caption{\cpnote{figura chafa. No se puede leer los numeros y ademas no entiendo que son las 
filas, las columnas, En fin. hablemos de esta figura}
Comparación de matrices de Choi objetivo, y matrices de Choi reconstruidas en el simulador cuántico \texttt{AerSimulator()} como en la computadora cuántica \texttt{ibm\_kyoto} respectivamente. \textbf{Fuente:} elaboración propia.}
...
\subsection{Implementación de VQA para 2\textit{qubit}s}
% }}}
\appendix
% Notas sueltas {{{
