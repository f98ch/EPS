...
\felnote{Por el momento está redactada la sección 1.2 y 1.4. Sigo redactando la sección 1.3 . }
\section{Introducción}
\felnote{Pendiente: la redactaré al finalizar el capítulo}

\section{Matriz Densidad}
\felnote{Sección heredada de informe de prácticas}
...
normalización \cite{nielsen_chuang_2011}:
    \begin{equation}
    \label{ec::1.2}
           |c_i|^2=\braket{\psi}{\psi}=1, 
    \end{equation} 
con $|c_i|^2$, como la probabilidad que el estado colapse al $i$-ésimo vector
$\ket{i}$ de la base al ser medido.

\begin{ex} \label{ex: dens1}\end{ex} Si tenemos un sistema de un  qubit
\footnote{Qubit es un sistema cuántico de dos estados (o dos niveles). Es la
...
\end{equation}
con $n$ el número de estados del ensamble y $p_i$ el 'peso' mencionado anteriormente, cumpliendo con la condición $\sum p_i =1$. \

Si damos una base ortonormal $\{\ket{i}\}$, con $i=1,2,...,d$ ($d$ es la
dimensión del espacio de Hilbert del sistema), veremos que los elementos de la matriz densidad serán \cite{princip_quantum}:
\begin{equation}
    \label{eq:rho_ij}
...
\end{align}
para cualquier matriz de dimensión $m\times n $ para $A$,  y $n \times p$ para $B$. Al ser $A = \ket{\psi_i}$ de dimensión $1\times n$ y $B = \bra{\psi_i}$ de dimensión $n \times 1$ podemos utilizar esta propiedad. Consecuentemente reescribimos la ecuación $\ref{ec:dagger}$ como
\begin{align}
...
\cite{princip_quantum} para la matriz densidad $\rho$ :
    ...
    \end{equation}
siendo $\ket{j}$ vectores  qubit zados y ortogonales y $d$ la dimensión del sistema, con  $\lambda_j \geq0$ ya que $\rho$ es positivo. Ya que la matriz es matriz diagonal, hacemos uso de la propiedad 2, obteniendo:
    \begin{equation}
 ...
 Usando la descomposición espectral nuevamente para $\rho^2$ tenemos:
    ...
    \end{align}
Como$\rho$ es diagonal concluímos: 
    \begin{equation}
    \label{eq:pureza_lambdas}
    \gamma=\sum_j\lambda_j^2.
    \end{equation} 
Por el procedimiento anterior con $\rho$ sabemos que $\sum_j \lambda_j = 1$ y$\lambda_j \geq 0 $. Por lo tanto, la condición $\gamma =1$ se cumple sí y solo sí $\lambda_j=1$ para un único $j = \overline{j}$, de lo contrario $\lambda_j=0$. Esto corresponderá a un estado puro.  
 Por otro lado, por definición sabemos que un estado es mixto si la representación diagonal (\ref{ec:dens_espectral}) de $\rho$ posee más de un estado puro, es decir $\lambda_j<1$ para todos los $j$. Por consiguiente:
    \begin{align}
        \sum_j \lambda_j^2 & < \sum_j \lambda_j =1 \\
        \therefore \gamma & < 1 \square
     \end{align}
 La demostración anterior prueba la cota superior de la pureza, para la cota inferior es necesario agregar un teorema:
 \par %Estos \par solo están para el \felnote
 \begin{thm}\textbf{(Desigualdad de Jensen):} \label{thm:jensen}  Sean $w_1,...,w_n$ números positivos que sumen $1$ y $f$ una función real continua convexa, entonces \cite{jensen_wolfram} :
\begin{equation}
...
Cuando trabajamos con un sistema conformado por dos o más sistemas físicos distintos se le conoce como un sistema compuesto.  El espacio de Hilbert correspondiente al estado que describe a este sistema , es el producto tensorial de cada uno de los espacios $H_i$ asociadados a los subsistemas: 
\begin{equation}
...
\end{equation}
a los que su matriz densidad correspondiente sería \cite{nielsen_chuang_2011} \footnote{Para este caso se utilizan números para indicar el estado $\rho$ que describe al $i$-ésimo sistema, esto también puede ser representado con letras mayúsculas tales como $\rho_A$ describiendo a un primer sistema $A$. En adelante se utilizarán letras mayúsculas como subíndices para mantener consistencia, y no confundirse con componentes de una matriz densidad como en la ecuación \ref{eq:rho_ij}.}: 
\begin{align}
    \rho_T  & = \rho_1 \otimes \rho_2 \otimes  ... \otimes \rho_n \\
            & = \ketbra{\psi_1} \otimes \ketbra{\psi_2} ... \otimes \ketbra{\psi_n}. 
\end{align}
Este no es el caso general para los sistemas compuestos, sin embargo, será con el que trabajemos en los siguientes capítulos.

Al trabajar con sistemas compuestos puede darse el caso donde no conozcamos uno, o varios de los estados que describan a sus subsistemas. Entonces, para dar una descripción a los subsistemas se usa la matriz densidad reducida. Utilizaremos la definición dada por Nielsen y Chuang \cite{nielsen_chuang_2011}:

Si tenemos a los sistemas físicos $A$,$B$ cuyo sistema compuesto se describe por el estado $\rho_{AB}$. Entonces el subsistema $A$ puede ser descrito por medio de la matriz densidad reducida $\rho_A$:
\begin{equation}
    \rho_A \equiv \text{Tr}_B(\rho_{AB}),
\end{equation}
con $\text{Tr}_B$ un mapeo de operadores, conocido como traza parcial, que actúa sobre el sistema compuesto. Si buscamos trazar parcialmente el subsistema $B$ obtenemos:
\begin{equation}
    \text{Tr}_B(\ket{i_A}\bra{j_A} \otimes \ket{i_B}\bra{j_B}  ) \equiv \ket{i_A} \bra{j_A} \text{Tr}(\ket{i_B}\bra{j_B}),
\end{equation}
siendo $\ket{i_A}$ y $\ket{j_A}$ un par de vectores cualquiera en el espacio de $A$, análogamente $\ket{i_B}$, $\ket{j_B}$ para el espacio de $B$. Por lo que la traza $tr$ en $B$ es:
\begin{equation*}
    \text{Tr}(\ket{i_B}\bra{j_B}) = \bra{j_B}\ket{i_B}.
\end{equation*}
Similarmente podemos calcular la matriz densidad para el sistema reducido $B$ trazando parcialmente $A$ como:
\begin{equation}
...
El estado que describe a este sistema es llamado un estado separable. Ya que al hallar sus subsistemas, veremos que puede escribirse como un producto tensorial $\rho_A \otimes \rho_B$, indicando que ambos subsistemas no estan correlacionados.  \par 
...
\begin{align}
    \rho_B  & = \text{Tr}_A(\rho_{AB})\\
        & =  \frac{1}{2} \left[ \text{Tr}_A(\ketbra{0_A0_B}) + \text{Tr}_A(\ketbra{1_A1_B}{0_A0_B}) + \text{Tr}_A(\ketbra{0_A0_B}{1_A1_B}) + \text{Tr}_A(\ketbra{1_A1_B}{1_A1_B})  \right] \\
...
\felnote{Heredado de informe de prácticas} 
...
\end{ex}
El canal despolarizante (\textit{Depolarizing Channel}) es un canal cuántico que introduce ruido aleatorio en un sistema, transformando cualquier estado cuántico de entrada en un estado mixto \cite{nielsen_chuang_2011}. La acción del canal actúa por una probabilidad $p$ de despolarizar un estado cuántico $\rho$ se describe de la forma:
...
\end{equation}
De esta forma podemos reconstruir la matriz $\rho$ como en la ecuación \ref{eq:tomo_estado}. \par 
...
(\ref{eq:fidelidad_estado}):
    ...
    \begin{equation}
        |\text{Tr}(AU)| \leq \sqrt{ tr|A| \text{Tr}(U^\dagger V^ \dagger  |A|VU) } = tr|A|,
    \end{equation}
...
\ref{ec:choi_depolarizing}.\cpnote{La referencia parece incompleta. Revisa con cuidado 
como referenciar un libro (es un libro?)}\felnote{Corregido: sigo redactando el cap1. Ya aparece la referencia en esta iteración.}
...
mediciones tanto en la computadora cuántica como en el simulador. \cpnote{frase rara y quedo como mal conectado
con lo que sigue} \felnote{Corregido: se verá reflejado desde 551 a 552} No incluimos 
...
\end{align}
\cpnote{La redaccion de este parrafo que contiene a las ecuaciones anteriores
está muy rara. Corrigela} \felnote{Corregido: se modificó la redacción del párrafo previo a las ecuaciones.}
...
con preservar la traza. Esto lo verificamos realizando la traza parcial de las matrices de Choi con \texttt{partial\_trace()}, obteniendo resultados que son cercanos a la matriz identidad $I$. Sin embargo, se observan valores no nulos en los elementos fuera de la diagonal, lo que impide que sean exactamente iguales a $I$ (ver figura \ref{fig:ident_comp}) .\cpnote{Aca si puede haber un problema más serio. Lo
platicamos.} \felnote{Pendiente: Noté que se quedan bastante cerca a los
valores de la identidad, pero siguen difiriendo. (ver imagen
comparaciones\_identidad\_reconstruida.png en la carpeta de imagenes del
proyecto) } \cpnote{Comenta que es muy cercana a la matriz identidad igual y el error
que se presenta} \felnote{Corregido: se verá reflejado en 592 a 595. Para reforzar el punto agregué la figura \ref{fig:ident_comp}}
\begin{figure}[h!] 
    \centering 
...
El aumento lineal de la fidelidad puede explicarse porque,\cpnote{Cual, todo? que sea lineal? que la xi sea baja?} \felnote{Corregido: El comportamiento lineal de la fidelidad, se verá en 620-621}
al aumentar $p$ en el DP, los estados se vuelven más
mixtos, reduciendo su sensibilidad a errores específicos del hardware cuántico. En sistemas como \texttt{ibm\_kyoto}, una alta despolarización reduce la sensibilidad del estado a errores de compuertas cuánticas y decoherencia, ya que un estado menos puro tiende a ser menos afectado por estas imperfecciones.  \cpnote{Interpretación
chafa}\felnote{Pendiente: reescribí este parrafo, quisiera saber tu opinión. (
334 a 341). } \cpnote{Esta mejor, pero aun admite una iterada. Otra cosa, al mover y editar
las lineas se mueven. Yo no se si se puede usar un comando de latex para 
marcar una linea. Quizá puedas probar. } \felnote{Corregido: iteré todo el párrafo (620-626). Por el lado de editar por línea, sigo buscando, si gusta puedo poner en la siguiente un label para referenciar en donde incia una correción.}
...
de $\chi^2 = 0.0633$. 
Aunque el bajo valor de $R^2$ indica que la relación entre la fidelidad y $p$ en el simulador es débil, sí se observa un aumento lineal sistemático en la fidelidad con respecto a $p$. La pequeña pendiente de la recta sugiere que el cambio en la fidelidad es leve y se mantiene prácticamente constante a lo largo del rango de $p$, aunque no completamente insensible al aumento de $p$. \cpnote{Estoy en desacuerdo. Si se observa que sistematicamente hay un aumento lineal. Quizá sea solo cosa de redaccion, pero lo platicamos.} \felnote{Corregido: Indiqué que sí, hay un aumento, aunque a comparación de la computadora cuántica es una tendencia menos pronunciada. }
...
introduce ruido desde un inicio del proceso de despolarización debido a las imperfecciones inherentes al hardware y las compuertas cuánticas.\cpnote{por?} \felnote{Corregido: el por qué va en la línea 675-676}. Notemos que al incrementar $p$, aumentamos la probabilidad de despolarización, lo cual introduce más ruido a la interacción del sistema con el estado máximamente mixto (ver ecuación \ref{ec:superop_depolarizing})\cpnote{Cual interaccion?} \felnote{Corregido: entre el sistema y el estado maximamente mixto por cómo se comporta el depolarizing.(corregido en 678 -679)}. Este proceso provoca una mejora en la fidelidad porque, al hacer que los estados sean menos puros, se vuelven menos sensibles a errores específicos del hardware cuántico. \cpnote{por?} \felnote{Corregido: el por qué esta de 682-683. }. 
...
reconstruidas]{\cpnote{Mejoro mucho. Deja dos figuras significativas y quita la barra que no se ve. Con los valores ya se entiende. Aumenta lo que aparece encima de cada matriz. Casi no se puede leer. Quizá para ganar espacio, puedes poner f= en vez de Fidelidad. Está bonita
y me gusta la idea.} \felnote{Corregido: muchas gracias, verás la fuente más grande de los subtítulos de cada matriz, usé $F_{proc}$ por la ecuación \ref{ec:fidel_proc}. Ya no aparece el colorbar.}
