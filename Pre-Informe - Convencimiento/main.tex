...
% }}}
\chapter{FUNDAMENTOS TEÓRICOS} \label{cap:1}% {{{
...
y dale un nuevo nombre a aesta parte} \felnote{Agregada}
\section{Algoritmos de implementación}
Se han programado las funciones necesarias para implementar un \textit{depolarizing channel} \cpnote{utiliza español. Si insistes en ingles, usa italicas y no 
le pongas maypusculas.} \felnote{corregido, usaré itálica. No he visto aún una traducción literal del depolarizing} mediante VQA en una computadora cuántica. Estas funciones siguen el algoritmo presentado en el capítulo anterior, desarrollado en el lenguaje Python con el apoyo esencial de la librería Qiskit \cpnote{Este es un nombre propio
y por consiguiente va con mayuscula. Cambia todos.} \felnote{Listo. Corregí todos los Python y Qiskit. } . A continuación, se enumeran
...
acorde a la teoría explicada en el \autoref{cap:1}\cpnote{Usa el comando ref
porque puedes meter uno que sea solo de intro sin teoria o quitar y te desparatoa
los numeors.} \felnote{Corregido.}. Toma como entrada el circuito
...
mayuscula o no. Quedo pendiente de lo que encuentres antes de proseguir} \felnote{Si se pueden usar dos puntos antes de una enumeración o ecuaciones.   Aquí hay un ejemplo en arxiv: https://arxiv.org/pdf/2401.00019. Por las mayúsculas luego de ":", según APA , se inicia con minúscula para este caso luego de los dos puntos, ya que  la descripción no es una oración nueva, sino una explicación del elemento.}
    \begin{enumerate}
...
defecto es para dos\textit{qubit}s).
...
parámetro. Se utilizaron 4 \textit{qubits} en el \textit{ansatz}, donde dos se usaron como
