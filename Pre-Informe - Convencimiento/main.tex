...
como referenciar un libro (es un libro?)}
...
número de 4000 mediciones por circuito. \cpnote{frase rara y quedo como mal conectado
con lo que sigue}
...
identidad $I$. \cpnote{Aca si puede haber un problema más serio. Lo
...
Este comportamiento \cpnote{Cual, todo? que sea lineal? que la xi sea baja?}
puede explicarse porque, al aumentar $p$ en el DP, los estados se vuelven más
mixtos y menos afectados por errores específicos del hardware cuántico. En
sistemas como \texttt{ibm\_kyoto}, una alta despolarización puede reducir el
impacto de errores de las compuertas cuánticas y la decoherencia, ya que el
estado menos puro es menos sensible a estos fallos. Entonces, la fidelidad
mejora con  $p$ porque el DP atenúa algunos errores del sistema, no porque el
hardware mejore su rendimiento.  \cpnote{Interpretación
...
marcar una linea. Quizá puedas probar. }
...
depender significativamente de $p$. Además, la
pendiente de la recta es
pequeña, lo que indica que la fidelidad en el simulador es prácticamente
constante a lo largo del rango de $p$. \cpnote{Estoy en desacuerdo. Si se observa que sistematicamente hay un aumento lineal. Quizá sea solo cosa de redaccion, pero lo platicamos.}
...
introduce ruido desde un inicio\cpnote{por?}. Notemos que al incrementar $p$, introducimos
más ruido a la interacción\cpnote{Cual interaccion?}, que hace que nuestra
fidelidad mejore con la
computadora cuántica\cpnote{por?}. Por otro lado, las matrices obtenidas del simulador son
