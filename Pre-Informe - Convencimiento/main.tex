...
$\ket{i}$ de la base al ser medido\cpnote{Al ser medido que? Lo discutimos para 
...
dimensión del espacio de Hilbert del sistema \cpnote{ya lo dijiste}), veremos que los elementos de la
...
Como $\rho$ es diagonal concluímos: 
    ...
    \end{equation} 
Por el procedimiento anterior con $\rho$ sabemos que $\sum_j \lambda_j = 1$
y $\lambda_j \geq 0 $. Por lo tanto, la condición $\gamma =1$ se cumple sí y
solo sí $\lambda_j=1$ para un único $j = \overline{j}$ \cpnote{no entiendo esa ecuacion}, de lo contrario
$\lambda_j=0$. Esto corresponderá a un estado puro.  
