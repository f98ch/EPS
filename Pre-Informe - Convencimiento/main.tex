...
donde $|c_i|^2$ representa la probabilidad de que, al medir el sistema, el estado colapse al $i$-ésimo vector $\ket{i}$ de la base. \cpnote{Al ser medido que? Lo discutimos para 
que no te vayas por un camino largo} \felnote{Corregido: 1. El sistema, lo agregué (línea . 2. Sí claro, me parece bien. }
...
Si damos una base ortonormal $\{\ket{i}\}$, con $i=1,2,...,d$  \cpnote{ya lo dijiste} \felnote{Corregido: removí la redundancia. (línea 110)}, veremos que los elementos de la
...
Por lo tanto, la condición $\gamma = 1$ se cumple si y solo si existe un único índice $\overline{j}$ tal que $\lambda_{\overline{j}} = 1$, y $\lambda_j = 0$ para todo $j \ne \overline{j}$. \cpnote{no entiendo esa ecuacion} \felnote{Corregido: reordene el flujo de la demostración. Ahora debe tener más sentido que para un estado puro, necesita tener un único autovalor=1 (la parte de los índices j), mientras que los mixtos tinen más de un autovalor<1. (135-150~)} Esto corresponde a un estado puro, ya que la matriz densidad se reduce a un proyector sobre un solo estado:

\begin{equation}
\rho  = \lambda_{\overline{j}} \ketbra{\overline{j}} = \ketbra{\overline{j}},
\end{equation}
que es la misma representación vista en la ecuación (\ref{ec:dens_puros}). Por otro lado, en un estado mixto, la matriz densidad $\rho$ tiene más de un autovalor $\lambda_j$ distinto de cero, todos en el rango $0 < \lambda_j < 1$. Por consiguiente:
    \begin{align}
...
el que trabajemos en los siguientes capítulos\cpnote{No creo qe trabajemos con este 
estado. lo discutimos} \felnote{Sí claro, me parece bien. Yo lo uso luego para introducir los canales (linea 330).}.
...
\cpnote{Escribe solo lo necesario aca por favor}
...
Previo a hablar sobre compuertas cuánticas, es necesario introducir una forma de representar geométricamente al qubit, por ello utilizamos la esfera de Bloch. Esta esfera será una herramienta que nos permitirá visualizar los cambios del estado de un qubit bajo diversas transformaciones unitarias $U$.

\begin{figure}[h]
    \centering
    \includegraphics[scale=0.70]{imagenes/Bloch.png}
    \caption{Representación de un estado cuántico en la esfera de Bloch. \textbf{Fuente:} Nielsen y Chuang \cite{nielsen_chuang_2011}}.
    \label{fig:Bloch}
\end{figure}
La represntación de un qubit en la esfera de Bloch, se describe por medio de la ecuación: 
\begin{equation}
    \label{ec:Bloch_rep}
...
\end{equation} 
La ecuación \ref{ec:Bloch_rep} nos permite describir estados puros en la esfera de Bloch, sin embargo, para visualizar cualquier estado $\rho$ de un qubit usamos el vector de Bloch. Este es un vector cuyas componentes señalan un punto en la esfera de Bloch \cite{princip_quantum}.
...
\end{matrix}\right. 
\end{equation}
El valor absoluto $|\textbf{r}|=1$ para estados puros, mientras que $|\textbf{r}|<1$ para estados mixtos. Notemos que los estados puros se encuentran en la superficie de la esfera de Bloch, mientras que los estados mixtos en el interior. 

Por lo tanto, obtenemos otra representación útil de la ecuación \ref{ec:Bloch_rep} mediante el operador $\rho$ para estados puros como :
...
$n\times n $. Estas actúan linealmente sobre qubits que pueden estar en superposición. Además, se visualizan por cajas que contienen las iniciales del operador aplicado (ver figura \ref{fig:compuerta}). 
...
Nos enfocaremos en un conjunto específico de compuertas de uno y varios qubits necesarias para este trabajo, comenzando con las compuertas de rotación en los ejes de la esfera de Bloch: $R_x$, $R_y$ y $R_z$. Estas compuertas rotan el estado del qubit alrededor de cada eje de la esfera de Bloch y pueden interpretarse como versiones parametrizadas de los operadores de Pauli (ver ecuación \ref{ec:pauli_matrix}). La representación de cada rotación es:
\begin{align}
...
Si aplicamos una rotación con un ángulo $\pi/2$ es equivalente a aplicar los operadores de Pauli $X,Y,Z$. Esto cambia la dirección del estado en su respectivo eje (ver figura \ref{fig:rep_pauli}). 
...
$\frac{(x+z)}{\sqrt{2}}$ en la esfera de Bloch.  También puede descomponerse como la sucesión de $\pi$ radianes en el eje $x$, seguido de una rotación de
$\pi/2$ en $y$. (ver figura \ref{fig:hadamard}).
...
Las mediciones en otras bases será de utilidad para la tomografía de estado ya que hace uso de distintas mediciones en distintas bases para recuperar la información de un estado cuántico.
% }}}
