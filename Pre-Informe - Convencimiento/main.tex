...
normalización \cite{nielsen_chuang_2011}:
    ...
    \end{equation} 
donde \cpnote{Fijate queu si dejas una linea en blanco en el tex entre la
ecuación y la siguiente linea, lo toma como un nuevo parrafo, y en este caso lo
identa. No queremos eso, por lo que no debes dejar esa linea en blanco. En
general lo voy a corregir, pero corrigelo aca y en lo que sigas
escribiendo.} \felnote{Está bien, lo ajusté para todas las ecuaciones.} $|c_i|^2$ es la probabilidad de encontrar al sistema en el estado $\ket{i}$ al medir el observable asociado con la base $\ket{i}$.
\cpnote{Al ser medido
que? Lo discutimos para que no te vayas por un camino largo}
\felnote{Corregido: 1. El sistema, lo agregué (línea . 2. Sí claro, me parece
bien. }\cpnote{Aun no entiendes el punto. Da lo mismo medir cualquier observable?} \felnote{Corregido: tienes razón, es medir el observable de la base del sistema. Lo modifiqué}
...
para introducir los canales (ec: \ref{ec:rep_sist_abierto} ).}\cpnote{Ahora que estoy quitando comentarios se mueve
todo. Busque odnde haces referencia a esta ecuacion pero no encuentro. Quizá si en vez de 
referir una linea, refieres una ecuacion es mas facil de encontrar} \felnote{Corregido: sí claro, ahora referenciaré a una ecuación o pondré un label para ubicar cambios.}.
...
% Intro {{{
Este capítulo estudiará las propiedades y definiciones esenciales de la computación cuántica para entender los circuitos cuánticos. Comenzaremos describiendo las compuertas cuánticas y su acción sobre estados de qubits, incluyendo cómo se transforman en la esfera de Bloch. También exploraremos las mediciones cuánticas y las transformaciones necesarias para medir en distintas bases. Finalmente, describiremos la plataforma de \textit{IBM Quantum Experience} donde se implementarán nuestros circuitos cuánticos, mencionando sus capacidades y limitaciones para entender cómo inicializar estados en circuitos cuánticos. Los conocimientos y técnicas
adquiridas a lo largo de esta sección nos darán las bases necesarias para
trabajar con estados y canales cuánticos en las próximas secciones. \label{introduccion} \cpnote{Falta una miniintro que puede ser de un par de frases posiblemente diciendo lo que vas a hablar y porque.} \felnote{Corregido: le agregué una introducción \ref{introduccion}}

% }}}
\subsection{Compuertas y mediciones cuánticas} % {{{

\cpnote{Revisa redaccion}\felnote{Corregido: revisé la redacción nuevamente. También le pedí ayuda a subadra para verificarla.}
...
\textbf{Fuente:} Nielsen y Chuang \cite{nielsen_chuang_2011}.}
    \label{fig:Bloch}
\end{figure}
La representación de un qubit en la esfera de Bloch esta dada por la ecuación :\cpnote{esta frase no tiene sentido. porque dices quue vamos a describir una representacion???} \felnote{Corregido: esto si fue error de redacción mío, en lugar de poner 'esta dada' había puesto 'se describe' (ahora debe decir : esta dada)}: 
\begin{equation}
    \label{ec:Bloch_rep}
...
\end{matrix}\right. 
\end{equation}
El valor absoluto $|\textbf{r}|=1$ para estados puros, mientras que $|\textbf{r}|<1$ para estados mixtos. Notemos que los estados puros se encuentran en la superficie de la esfera de Bloch, mientras que los estados mixtos en el interior. Por lo tanto, obtenemos otra representación útil de la ecuación \ref{ec:Bloch_rep} mediante el operador $\rho$ para estados puros como :
