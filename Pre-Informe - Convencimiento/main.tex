...
*Sección heredada de informe de prácticas
\section{Tomografía y Fidelidad de Proceso}
La tomografía de proceso cuántico (QPT) es una técnica para reconstruir experimentalmente un canal cuántico mediante mediciones. Primero, entendemos la tomografía de estado cuántico, que permite reconstruir un estado desconocido a partir de mediciones repetidas.

Para implementar la tomografía de estado en una computadora cuántica, realizamos mediciones en las bases de los operadores de Pauli $(X,Y,Z)$. Usamos compuertas cuánticas como Hadamard $H$ y $S^{\dagger}$ para cambiar las bases de medición.

Con esta información, pasamos a la tomografía de proceso, que implica preparar un conjunto de estados puros de entrada, pasarlos por el canal $\mathcal{E}$ y realizar tomografía de estado en las salidas. Esto nos permite reconstruir la matriz de Choi $\Lambda_{\mathcal{E}}$ del canal cuántico.

La fidelidad de estado cuántico mide la similitud entre dos estados $\rho$ y $\sigma$. Utilizamos esta métrica para definir la fidelidad de proceso cuántico $F_{\text{pro}}(\mathcal{E}, \mathcal{F})$, que compara dos canales cuánticos. Esta métrica es útil para medir la diferencia entre un canal cuántico teórico y uno experimental.
