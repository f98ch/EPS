...
\cpnote{Le quite el nombre de introduccion. Creo que está mejor. Quita igual esto para los otros }\felnote{Listo, corregido}
...
$cR_y$.  En este esquema, los parámetros $\bm{\theta}$ corresponden a los ángulos de rotación de las compuertas del ansatz. Este circuito puede interpretarse como un modelo de sistema abierto, donde el sistema principal interactúa con un entorno, como se muestra en la ecuación ~\ref{ec:rep_sist_abierto}.
...
\cpnote{La redaccion de esta frase esta muy rara. Que 
quieres decir aca?} \felnote{Sí esta rara, modifiqué esa frase (591-595). Aquí quiero decir que este ansatz representa el sistema como $\rho_{sp} \otimes \rho_{ent}$ ( $\rho_{sp}$ es el sistema principal) . Lo menciono para darle sentido a la ecuación  (\ref{ec:tr_anc}) }.  Notemos que este ansatz construye un estado puro
de  $2n$ qubits de la forma:
\begin{equation} 
...
\end{equation}
donde $\rho(\bm{\theta})$ representa el estado del sistema luego de su interacción con el canal cuántico. Una vez definida la función de costo en la ecuación~\ref{ec:cost_func}, se procede a ejecutar la tomografía de proceso sobre $\rho(\bm{\theta})$ con el propósito de reconstruir la matriz de Choi $\Lambda_{r}$, que será comparada con la matriz de Choi del canal objetivo $\Lambda_{obj}$ mediante la fidelidad de proceso \cpnote{mejorar la redaccion} \felnote{Corregida}.  Este procedimiento se repite iterativamente, actualizando los parámetros $\bm{\theta}$, hasta encontrar el conjunto óptimo:
\begin{equation}
...
\cpnote{mejora la redaccion de este parrafo. Por cierto, para que usas el begin ex?} \felnote{Para iniciar ejemplos, usé el mismo formato que JA.} \felnote{Corregido, arreglé la redacción.}
...
Iniciamos los parámetros iniciales $(\bm{\theta_0})$ de forma aleatoria y ejecutamos el VQA hasta alcanzar la minimización de la función de costo (\ref{ec:argmin_cost})(ver esquema en la figura \ref{fig:esquema_dp}). Notemos que esta matriz de Choi, se debe específicar para un valor de $p$. En consecuencia, se obtiene la matriz $\Lambda_r(p)$.
\begin{figure}[h!] 
    \centering 
...
\cpnote{Porque no se logra optimizar al maximo??? Osea a fidelidad uno?}\felnote{Porque el optimizador clásico no garantiza encontrar un máximo global. En ningún caso que ejecuté alcancé un máximo global, pero si uno local (por ejemplo, mira los resultados de la figura \ref{fig:comparacion_capas}) . Por eso en redacción lo dejé más abierto con 'optimizada'.}
\begin{equation}
...
\end{pmatrix},
\end{equation}
que corresponde a una fidelidad de proceso $F_{proc}= 0.9431$. \cpnote{Redaccion. 
Esta usando el ``por lo que'' de manera recurrente y no está bien.} \felnote{Gracias. En esta frase ví que incluso se puede omitir, revisé y arregle todas la instancias donde lo veía raro.} El
