...
Creo qeu si se debe comentar en la tesis.} \felnote{Corregido en base a lo que platicamos por llamada, (ver el siguiente felnote) }
\begin{equation}
...
\end{pmatrix},
\end{equation}
que corresponde a una fidelidad de proceso $F_{proc}= 0.9431$. Notemos que el circuito resultante del ansatz optimizado para este valor de $p$ (ver figura \ref{fig:ansatz_p025}) no alcanza una fidelidad de proceso igual a uno. \felnote{aquí empieza esa mención }Esta diferencia se debe a que la obtención de la matriz de Choi se realiza mediante tomografía de procesos, la cual implica mediciones repetidas en el simulador, introduciendo errores asociados al muestreo, a diferencia de un cálculo analítico exacto. \felnote{aquí finaliza} \begin{figure}[h!] 
    \centering 
...
En este trabajo propusimos el uso de algoritmos variacionales cuánticos (VQA,
por sus siglas en inglés) para implementar canales cuánticos de un qubit en
computadoras cuánticas. Se utilizó un \textit{hardware-efficient ansatz},
optimizado mediante un simulador cuántico (\texttt{AerSimulator}) y validado en
una computadora cuántica de IBM (\texttt{ibm\_kyoto})
\cite{VQA,ibm_quantum_resources}. La optimización se llevó a cabo maximizando
la fidelidad de proceso entre las matrices de Choi correspondientes al canal
objetivo y al canal implementado, obtenidas a partir de tomografía de procesos.
Como caso de estudio, se consideró la simulación de un canal despolarizante
(DP) para distintos valores de la probabilidad de despolarización $p$. Los
resultados obtenidos muestran que el simulador cuántico es capaz de implementar
la acción del DP con fidelidades cercanas a 1. Además, se observó una baja
variabilidad como correlación con el valor $p$, que refleja una notable
estabilidad con el entrenamiento\cpnote{Simplifica y corrige la redaccion}. Por otro lado, los resultados obtenidos en la
computadora cuántica presentaron un comportamiento distinto. En este caso, la
fidelidad tendió a aumentar conforme incrementaba la interacción con el ruido
del dispositivo (ver figura  \ref{fig:comparacion_fidelidad}). Esta tendencia
fue medida mediante una regresión lineal , cuya pendiente resulto ser $m=
0.2707$, que indica un crecimiento moderado de la fidelidad con $p$. Así mismo,
observamos una disminución en la variabilidad de la fidelidad con el aumento de
$p$. Este fenómeno puede atribuirse a que, al aumentar la probabilidad de
...
coeficiente de terminación $R^2=0.7055$ , sugiere que existen otros factores,
además de la probabilidad de despolarización, que influyen en el comportamiento
de la fidelidad en el dispositivo cuántico. Estos factores se evidenciaron en
la inspección de las matrices de Choi (ver figura \ref{fig:comparacion_choi}) ,
