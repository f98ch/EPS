...
resultado : \cpnote{Porque no se logra optimizar al maximo??? Me gustaria platicar al respecto. 
Creo qeu si se debe comentar en la tesis.} \felnote{Corregido en base a lo que platicamos por llamada, (agregué la mención del cálculo analítico).  }
\begin{equation}
...
\end{pmatrix},
\end{equation}
que corresponde a una fidelidad de proceso $F_{proc}= 0.9440$. Notemos que el circuito resultante del ansatz optimizado para este valor de $p$ (ver figura \ref{fig:ansatz_p025}) no alcanza una fidelidad de proceso igual a uno. \felnote{aquí empieza esa mención } Esta diferencia se debe a que en primer lugar, el optimizador finalizar al alcanzar un mínimo local. Por otro lado la obtención de la matriz de Choi se realiza mediante tomografía de procesos, la cual implica mediciones repetidas en el simulador, introduciendo errores asociados al muestreo. Si realizamos un cálculo analítico del circuito optimizado de la figura \ref{fig:ansatz_p025}, la matriz de Choi resultante sería:
\begin{equation}
...
\end{pmatrix},
\end{equation}
que tiene una fidelidad de proceso $F_{proc} = 0.9547$. Esta es superior al obtenido con la tomografía de proceso. \felnote{aquí finaliza} \begin{figure}[h!] 
    \centering 
...
presentado en el ansatz \ref{ec:ansatz} \felnote{Agregué la referencia en esta última iteración}. Esto se decidió porque al
...
En este trabajo propusimos el uso de algoritmos variacionales cuánticos (VQA por sus siglas en inglés) para implementar canales cuánticos de un qubit en computadoras cuánticas. Se usó un \textit{hardware-efficient ansatz}, optimizado con un simulador cuántico (\texttt{AerSimulator}) y validado la computadora cuántica \texttt{ibm\_kyoto} \cite{VQA,ibm_quantum_resources}. La optimización buscó maximizar la fidelidad entre las matrices de Choi del canal objetivo y del canal implementado, calculadas mediante tomografía de procesos. Como caso de estudio, se simuló un canal despolarizante (DP) para distintos valores de la probabilidad de despolarización $p$. Los resultados muestran que el simulador alcanzó fidelidades cercanas a 1. Además, la variabilidad, como la correlación de la fidelidad con $p$ fue baja, lo que refleja un comportamiento estable del entrenamiento. \cpnote{Simplifica y corrige la redaccion} \felnote{Corregido}. Por otro lado, los resultados obtenidos en la computadora cuántica presentaron un comportamiento distinto. La fidelidad tendió a aumentar conforme crecía la interacción con el ruido del dispositivo (ver figura  \ref{fig:comparacion_fidelidad}). Analizamos esta tendencia con una regresión lineal , cuya pendiente fue $m = 0.2707$, que indica un crecimiento moderado de la fidelidad con $p$. También observamos una disminución en la variabilidad de la fidelidad al aumentar $p$. Este fenómeno puede atribuirse a que, al aumentar la probabilidad de
despolarización, los estados cuánticos se vuelven más mixtos y, por lo tanto,
más susceptibles a errores introducidos por el hardware. Sin embargo, el
coeficiente de determinación $R^2=0.7055$  sugiere que existen otros factores,
además de $p$, que influyen en el comportamiento
de la fidelidad en el dispositivo cuántico. Estos factores se evidenciaron al inspeccionar las matrices de Choi (ver figura \ref{fig:comparacion_choi}) ,
donde la computadora cuántica presentaba una mayor coherencia en las entradas
no diagonales en las primeras etapas del proceso. Finalmente, los resultados
obtenidos muestran que en entornos ideales, los VQA pueden ser efectivos para
implementar canales cuánticos. No obstante, en computadoras cuánticas reales,
la precisión se ve afectada por la naturaleza del hardware, ya que este
introduce una fuente adicional de ruido que debe ser considerado al momento de
la implementación. 
% }}}
