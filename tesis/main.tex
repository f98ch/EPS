...
% El estudio de la dinámica de sistemas cuánticos abiertos es esencial para el desarrollo de la computación e información cuántica. A diferencia de los sistemas cerrados, donde la evolución está completamente determinada por operadores unitarios, los sistemas abiertos requieren una descripción más general que considere la interacción del sistema con su entorno. En este contexto, los canales cuánticos permiten modelar dicha evolución mediante mapas completamente positivos y que preservan la traza (CPTP), convirtiéndose en una herramienta central para caracterizar procesos ruidosos y operaciones no ideales en plataformas cuánticas reales.

% Con el avance de tecnologías de propósito cuántico, especialmente en la era NISQ (Noisy Intermediate-Scale Quantum), se vuelve crucial encontrar métodos que permitan implementar y validar estos canales en dispositivos con recursos limitados y expuestos a ruido. Entre las técnicas más prometedoras para enfrentar estos desafíos se encuentran los Algoritmos Variacionales Cuánticos (VQA), que combinan circuitos cuánticos parametrizados con estrategias de optimización clásica. Este enfoque híbrido ha demostrado ser eficaz para tareas como la estimación de estados, simulaciones y, en particular, para la caracterización y síntesis de canales cuánticos.

% En este trabajo, se aborda la implementación del canal despolarizante de un qubit utilizando VQAs, enfocándose en la inicialización del canal mediante circuitos cuánticos y su posterior optimización a través de métricas como la fidelidad de proceso. El procedimiento se basa en la reconstrucción experimental del canal usando tomografía de proceso cuántico, contrastando los resultados con simulaciones ideales y evaluaciones realizadas en una computadora cuántica real. Este enfoque busca explorar la viabilidad de los VQAs como herramientas para diseñar y controlar canales cuánticos de forma adaptable y eficiente, abriendo camino hacia aplicaciones más robustas en computación cuántica bajo condiciones reales.
...
bien conectados} \felnote{No lo es. Último bloque pendiente por redactar.}
% }}}
% \felnote{Corregido: Introducción redactada}
% {{{
% % Intro {{{
...
\cpnote{Falta introduccion. Escribirla al final quizá de la tesis}\felnote{Corregido: Se agregó introducción a este capítulo}
% }}}
