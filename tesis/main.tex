...
\ref{fig:ansatz_p025}) no alcanza una fidelidad de proceso igual a uno.
Esta diferencia se debe a que, en primer
lugar, el optimizador finaliza al alcanzar un mínimo local. En segundo lugar, la
obtención de la matriz de Choi se realiza mediante tomografía de proceso, la
cual implica mediciones repetidas (pero finitas) en el simulador, introduciendo
errores asociados al muestreo. Si realizamos un cálculo analítico del circuito
optimizado de la figura \ref{fig:ansatz_p025}, la matriz de Choi resultante
sería\cpnote{no está claro de esta frase si es de mediciones o analitico. 
pense que el anterior era el analitico... creo qeu tienes que ser mas 
claro}:
\begin{equation}
...
\end{pmatrix},
\end{equation}
que tiene una fidelidad de proceso $F_{proc} = 0.9547$. Esta es superior al
obtenido con la tomografía de proceso. 
\begin{figure}[h!] 
    \centering 
