...
\cpnote{Le quite el nombre de introduccion. Creo que está mejor. Quita igual esto para los otros }
...
$cR_y$, por lo que en este caso los parámetros $\bm{\theta}$ son rotaciones de
las compuertas del ansatz.  Podemos ver a este circuito cuántico como un
...
\ref{ec:sistema_principal} \cpnote{La redaccion de esta frase esta muy rara. Que 
quieres decir aca?}.  Notemos que este ansatz construye un estado puro
...
Ya que hemos definido la función de costo en la ecuación \ref{ec:cost_func}, se ejecuta la tomografía de proceso  sobre $\rho(\bm{\theta})$ para encontrar $\Lambda_{r}$ y comparar su fidelidad de proceso con la matriz de Choi del canal objetivo $\Lambda_{obj}$\cpnote{mejorar la redaccion}. Este proceso  se repite iterativamente como lo vimos en la sección anterior hasta que se alcancen los parámetros óptimos de la forma:
\begin{equation}
...
\cpnote{mejora la redaccion de este parrafo. Por cierto, para que usas el begin ex?}
...
\cpnote{Porque no se logra optimizar al maximo??? Osea a fidelidad uno?}
\begin{equation}
...
Esta usando el ``por lo que'' de manera recurrente y no está bien. } Por lo que el
