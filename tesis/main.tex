...
En este trabajo propusimos el uso de algoritmos variacionales cuánticos (VQA, por sus siglas en inglés) para implementar canales cuánticos de un qubit en computadoras cuánticas. Se utilizó un \textit{hardware-efficient ansatz}, optimizado mediante un simulador cuántico (\texttt{AerSimulator}) y validado en una computadora cuántica de IBM (\texttt{ibm\_kyoto}) \cite{VQA} \cite{ibm_quantum_resources}. La optimización se llevó a cabo maximizando la fidelidad de proceso entre las matrices de Choi correspondientes al canal objetivo y al canal implementado, obtenidas a partir de tomografía de procesos. Como caso de estudio, se consideró la simulación de un canal despolarizante (DP) para distintos valores de la probabilidad de despolarización $p$. Los resultados obtenidos muestran que el simulador cuántico es capaz de implementar la acción del DP con fidelidades cercanas a 1. Además, se observó una baja variabilidad como correlación con el valor $p$, que refleja una notable estabilidad con el entrenamiento. Por otro lado, los resultados obtenidos en la computadora cuántica presentaron un comportamiento distinto. En este caso, la fidelidad tendió a aumentar conforme incrementaba la interacción con el ruido del dispositivo (ver figura  \ref{fig:comparacion_fidelidad} ). Esta tendencia fue medida mediante una regresión lineal , cuya pendiente resulto ser $m= 0.2707$, que indica un crecimiento moderado de la fidelidad con $p$. Asímismo, observamos una disminución en la variabilidad de la fidelidad con el aumento de $p$. Este fenómeno puede atribuirse a que, al aumentar la probabilidad de despolarización, los estados cuánticos se vuelven más mixtos y, por lo tanto, más susceptibles a errores introducidos por el hardware. Sin embargo, el coeficiente de terminación $R^2=0.7055$ , sugiere que existen otros factores, además de la probabilidad de despolarización, que influyen en el comportamiento de la fidelidad en el dispositivo cuántico. Estos factores se evidenciaron en la inspección de las matrices de Choi (ver figura \ref{fig:comparacion_choi}) , donde la computadora cuántica presentaba una mayor coherencia en las entradas no diagonales en las primeras etapas del proceso. Finalmente, los resultados obtenidos muestran que en entornos ideales, los VQA pueden ser efectivos para implementar canales cuánticos. No obstante, en computadoras cuánticas reales, la precisión se ve afectada por la naturaleza del hardware, ya que este introduce una fuente adicional de ruido que debe ser considerado al momento de la implementación. 


\chapter{TRABAJO FUTURO}
Los resultados obtenidos en la computadora cuántica abren diversas líneas de investigación para trabajos futuros. A continuación, se proponen algunas de ellas: 
\begin{itemize}
